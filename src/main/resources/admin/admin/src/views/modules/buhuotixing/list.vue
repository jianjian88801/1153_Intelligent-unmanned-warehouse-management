                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('buhuotixing','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/wurenchangku/upload/buhuotixingMuBan.xls"
                        >批量导入补货提醒数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('buhuotixing','导入导出')"
                                ...
                                :on-success="buhuotixingUploadSuccess"
                                :on-error="buhuotixingUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('buhuotixing','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入补货提醒数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('buhuotixing','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "buhuotixing.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                                     ...
                                     label="操作">
                        <template slot-scope="scope">
                            <el-button v-if="isAuth('buhuotixing','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('buhuotixing','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('buhuotixing','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
                            <el-button v-if="isAuth('buhuotixing','申请') && scope.row.buhuotixingStautsTypes == 1" type="danger" icon="el-icon-delete" size="mini" @click="shenqingbuhuo(scope.row.id)">申请补货</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
            ...
            form:{
                id : null,
                buhuotixingName : null,
                buhuotixingTypes : null,
                buhuotixingNumber : null,
                buhuotixingStautsTypes : null,
                buhuotixingDelete : null,
                createTime : null,
            },
            dataList: [],
            pageIndex: 1,
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :"buhuotixing",//当前表表名,
                        sumColum : 'buhuotixing_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'buhuotixing_name',//分组字符串字段
                        // types : 'buhuotixing_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                params['buhuotixingDelete'] = 1// 逻辑删除字段 1 未删除 2 删除


                this.$http({
                    url: "buhuotixing/page",
                    method: "get",
                    params: params
                }).then(({data}) => {
                    if(data && data.code === 0){
			...
			shenqingbuhuo(id){
				let params={
					buhuotixingId:id,
					buhuoshneqingYesnoTypes:1
				}
				this.$confirm(`确定申请补货吗?`, "提示", {
				    confirmButtonText: "确定",
				    cancelButtonText: "取消",
				    type: "warning"
				}).then(() => {
				    this.$http({
				        url: "buhuoshneqing/save",
				        method: "post",
				        data: params
				    }).then(({data}) => {
				        if(data && data.code === 0){
            ...
            // 导入功能上传文件成功后调用导入方法
            buhuotixingUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "buhuotixing/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
